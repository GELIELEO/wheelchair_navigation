...
class OdometryPublisher
{
	...
	private:
		...
		double x,y,th,dt;
		ros::Time pre_time=ros::Time::now(), current_time
...
OdometryPublisher::OdometryPublisher()
{
	...
	enc_sub = nh.subscribe<geometry_msgs::Twist>("/cmd_vel_mux/input/teleop", 10, &OdometryPublisher::encoderCallback, this);
	odom_pub = nh.advertise<nav_msgs::Odometry>("odom", 50);
}

void OdometryPublisher::encoderCallback(const geometry_msgs::Twist::ConstPtr encoders)
{
	// unpack the encoder message in base_link frame
	current_time = ros::Time::now();
	double dt = current_time - pre_time;
	pre_time = current_time;
	double dx = encoders->linear.x*dt;
	double dy = encoders->linear.y*dt;
	double dth = encoders->angular.z＊dt;
